...
The Compiler aspect provides a single interface for Component compilation, [during development](/) and [during build](/).
It enables you to use the same commands and APIs to compile any Component, regardless of the specific compiler it uses.

:::info
The specific compiler used by a Component ([Babel](/), [TypeScript](/), and so on) is set by the Component's Env.
:::
