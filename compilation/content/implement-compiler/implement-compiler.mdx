...
Make sure your compiler is compiled, before using it:

```bash
bit compile
```

The use the new compiler, create an Env extension or use an existing one.
...
The compiler can be implemented using React's `overrideCompiler` and `overrideBuildPipe` APIs, however, these may not exist in other Envs.
