...
Env Services are applied on components via the components' [Envs](/envs/overview). That means, components of different types (which use different Envs) will use a different set of tools and configs for their development.
