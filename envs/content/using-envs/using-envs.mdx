...
Each Bit component has a Component Development Environment defined. This is required for Bit to know how to manage the component's lifecycle.  
...
Rerun `env set` with either a specific vesion or none to upgrade to the latest, or choose a specific version to update/downgrade to.

```
bit env set acme.demo/welcome acme.envs/react@4.0.0
```

## Manage envs for many components

A single workpsace may contain many components where components may have different envs. See a full list of which env is set for a component:
...
Component should not have more than a single env applied. If you found yourself with an issue in Bit where `status` retunrs the following:
