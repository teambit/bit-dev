...
Lanes to collaborate on changes across multiple components without interfering with the main development course. Lane is a pointer to one or multiple specific versions of components.  

You can use Lanes to record changes across a set of components, for a bug-fix, new feature, refactor or even for creating new components.   

You can include affected components of that change. It gives you a change to simulate how a single change can affect the entire code base, even for components that are outside of your workspace, making collaboration easy and scalable.  
