...
Defining a dependency policy in the Workspace can be done by configuring the `teambit.dependnecies/dependency.resolver` in the `workspace.jsonc` file. 
...
To learn more about how Bit resolves dependencies, head over to the [Depedency Resolution](/).
...
Bit automatically resolves dependencies as `devDependencies` when the files using them are classified as [Dev Files](/components/dev-files). By default, different aspects of
...
:::tip
Using different versions of peer dependnecies is dependent on the use case and can be achieved through various techniques.
