...
Component dependencies are defined in Bit through dependency policies. A dependency policy is definition of a set of dependencies, with a [SemVer](https://semver.org) range, allowed to use from components 
in your development workspace. Dependencies are set to components upon usage through an `import` or a `require` statement. 

Defining a dependency policy in the Workspace can be done by configuring the `teambit.dependencies/dependency.resolver` key in `workspace.json`.
...
In the example above, we are configuring `lodash` and `classnames` to use specific SemVer ranges. Every installation of dependencies using `bit install` command would install
...
To see the new dependency defined on my component, you can use `bit show`.
...
To learn about how Bit resolves dependencies, head over to the [Dependency Resolution](/) docs.
...
only for component development purposes. In the policy, dev dependencies are set just like any other dependency unlike in the `package.json` dependencies entry like in the example
below.
...
### Prefer wide ranges
Best practice to prefer the widest range of ranges for peer dependencies, that allows component consumers maximum flexibility using the component easily from variety of React
versions that might be of a constraint. Of course, the component has to be compliant with all supported versions defined in the range, and breaking changes are an important 
...
:::tip
Using different versions of peer dependencies is depending on the use case and can be achieved through various techniques
For example, React 17+ components using React hooks can be used in the same process through the Adapter pattern method. We offer variety of components
to address different cases but in general, it is not recommended to do, as it might decrease performance and downgrade dev experience for component consumers.
:::
