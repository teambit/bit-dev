...
Dependency policies can be applied on a specific group of components. This is done using the [`@teambit.workspace/variants`](/workspace/variants#merging-configurations) configuration API.
...
To learn how to select components using `@teambit.workspace/variants`, [see here](/workspace/variants#merging-configurations).
:::
...
:::caution
Dependencies can be directly configured as `devDependencies` only by nesting the dependency policies inside [variants](/workspace/variants#merging-configurations).
:::
