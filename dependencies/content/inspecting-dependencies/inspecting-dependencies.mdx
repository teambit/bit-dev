...
In Bit dependency management is automated. Bit reads your code to find all `import` statements and builds a dependency graph.  
You can use various tools to inspect the generated dependency graph to learn how your components are composed, assisting you in making architectural decisions. Moreover, as you can take control and define dependency policy for each components, by inspecting dependencies, you may see the result dependency graph and decide if this fits your desired outcome.
...
The first and most simple way to see component dependencies is using the local dev server.

```bash title="start dev server"
bit start
```

In each component page you can explore dependencies through the **Dependencies** tab. Here you see all components composed into your code.

import devServerDepsGraph from './dev-server-deps-graph.png';

<img src={devServerDepsGraph} />
...
<img src={devServerDepsCode} />

## Dependency Graph in the CLI

You can use the CLI to see all dependencies when running the `bit show` command.
...
To print the complete graph representation of the component in the CLI run `bit dependencies`:
...
Bit keeps track of the entire dependent graph in your workspace, so you can understand how a change in a component affets the rest of the codebase. 
use the `bit dependents` command:
