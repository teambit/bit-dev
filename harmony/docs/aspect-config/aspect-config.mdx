...
Aspect can be configured anywhere that accepts Harmony configuration. In Bit, it could be the Workspace, Scope and a Component. Also
it might come up if creating an Harmony application and configuring aspects to it.

## Configuring an Aspect

Configuring either Bit core aspects or external aspect is just the same.
...
External aspects that are configued on a Bit configuration, would be automatically and lazily downloaded as needed.

Aspects can also be configued to components using the `aspect` command.
...
## Default config
In cases where a property was not configured by the aspect consumer, the propery would be `undefined` or equal 
