...
Aspects are components used to extend Bit. Every Aspect is a component that uses the [Aspect environment](/).
You can use Aspects not just to extend Bit but also to built your own composable applications, that is your choice and we have no preference.
...
Your new component incldues four new files: `hello-world.aspect.ts`, `hello-world.main-runtime.ts` and regular documantation, compositions and test files.
...
Since Bit is entirely build and composed from Aspects. That means every feature in Bit is first available as an API, and you can use these
Aspects to extend Bit and build amazing development and collaboration expereinces on top.
...
These are few examples to core components in the Bit toolchain that you can use as an API:
...
To use the CLI aspect for example, in order to extend Bit with a new command is as simple as following.
...
In the same sense you can use the component aspect to obtain programmatic access to component objects, and to the generator aspect, in order to 
register a new [component generation template](/docs/generator) for use. 

Aspects like the below can be quite useful in various cases. You can use the GraphQL aspect to extend Bit with a new GraphQL schema, Express to
...
Our new `worldName` property can be provided in any configuration. For example in `workspace.json` or `scope.json`.
...
```
This will make the `worldName` configured here to be injected to the correspoding aspect.
...
You can use Slots as a utility provided by Harmony to create pluggable APIs. To [learn on Slots head here](/docs/extending-bit/slots).
