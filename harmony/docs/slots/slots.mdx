...
We use slots to provide the ability for API consumers to inject logic and behaviour. For example, behind the scenes, the CLI's aspect 
...
Slots are indexing objects by their corresponding [Component ID](/docs/components/component-id) and provide an API
