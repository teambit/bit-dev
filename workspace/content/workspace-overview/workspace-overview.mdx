...
Use the Bit workspace to build new component, change existing components, or import components and suggest changes for them, and then export changed or new components back to remote scopes.  
Workspaces are framework-agnostic. A single workspace can have components of all kinds; Angular, React or Node, and even a combination of all. Workspace are flexible to fit any workflow, from a single developer maintaining only Bit components or alongside other tools and apps.
...
Import any component and get a monolithic-like developer experience with ability to refactor many components in a single place. The workspace takes care of managing each component as an independent item with its own implementation, history and config ([read more](/docs/components/component-overview)).  
This allows you to be flexible with how you structure a workspace and its contents. For example, you can [import a component](/docs/components/importing-components) from anywhere and work on it, just like you built it in your workspace.
...
You can maintain components of different types and frameworks in a Bit workspace, as each component has its own configured [Component Development Environment](/docs/envs/envs-overview), which the workspace uses to run various workflows.
