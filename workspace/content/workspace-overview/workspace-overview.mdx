...
The Bit Workspace is made for component-driven development - it is where you build independent, loosley-coupled components and composing them to systems.  Workspace a composition of components. You can `import` any component to it for maintaining it, or `export` others for different teams to use as an API. Each component holds the implementation, history and config for itself, meaning the component is essentially de-coupled from the workspace.
...
- no need to handle per-component configs
