...
The Bit Workspace is made for component-driven development. It is where you build independent, loosley-coupled, components and composing them to systems.  
Workspace is a composition of components where a component is like a "mini repository". Each component manages implementation, history and config for itself. A component is decoupled from the workspace, you can `import` any Bit component or `export` yours.
...
Keeping a similar dev-experience across workspaces allows you to jump in and out from codebases. When you don't need to deal with a per-project setup or learn the workflow commands/scripts you simply focus on the code.
...
The workspace is a container for the components. You get a monolithic-like developer experience and ability to refactor many components in a single place. In this "container" a component manages its own implementation, history and config ([read more](/docs/components/component-overview)).  
This allows you to be flexible with how you structure a workspace and its contents. For example, you can [import a component](/docs/components/importing-components):
