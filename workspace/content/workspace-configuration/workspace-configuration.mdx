...
For example, the custom Aspect `company.scope/extensions/my-dev-files` defines files that match the glob pattern '**/*.my-dev.*' as [dev files](/docs/components/dev-files). 
Adding that Aspect to the workspace configuration will apply it on all components in the workspace (which will set every component's `*.my-dev.*` file as a dev file).
