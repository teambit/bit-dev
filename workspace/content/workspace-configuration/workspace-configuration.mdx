...
Since each Aspect is a [Component](/components/components-overview), it uses the [Component ID](/components/component-id) pattern.
...
For example, the custom Aspect `company.scope/extensions/my-dev-files` defines files that match the glob pattern '\*_/_.my-dev._' as [dev files](/components/dev-files).
