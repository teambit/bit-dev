...
A Component Directory is a directory for a Component in the [Workspace](/workspace/workspace-overview). The mapping of Components to their directories can be found in the [.bitmap](/workspace/structure/bitmap) file (see the `rootDir` property).

Every component directory must include a [Main File](/components/structure/component-main-file) (an entry file). The directory can be located anywhere in the Workspace (directory), excluding only the [node_modules](/dependencies/node-modules) directory and other Component Directories.
