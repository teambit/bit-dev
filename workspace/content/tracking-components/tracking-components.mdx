...
When initializing a Bit Workspace in a pre-existing project you can convert a existing directories to be Bit components. `add` command defines the [component directory](/docs/workspace/component-directory) and start the component isolation process. The command analyzes the source files to build the component's dependencies. Tracking a component adds it to the [`.bitmap`](//docs/workspace/bitmap) in the workspace.
