...
Aspects configured in the workspace config are applied on the workspace whereas, aspects configured in the component config are applied on the component.
They both may have a similar effect on the workspace components, but only in the context of that specific workspace.

Components exported from one workspace and imported into another, will lose their configuration, since the new workspace does not have the same Aspects applied on it. This is not the case for aspects that are applied on components (i.e, via the component config).
...
One of this workspace's components is `ui/button`. As you can see, in both cases `button.snapshot.png` is successfully defined as a development file. This will be persisted when the component is tagged or snapped.
