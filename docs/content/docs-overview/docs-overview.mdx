...
Doc files, of different formats, are loaded from the workspace's [component directories](/workspace/structure/component-directory), and displayed using the [doc template](/dev-services-overview/docs/docs-templates) that was set by each component's Env.
