...
You can intialize Bit on any exsiting workspace to start onboarding components from it to Bit. Bit has a minimal footprint on your project and can run alongside existing tools.  
Start onboarding components by heading over to your project root:
...
- [`.bitmap`](/docs/workspace/bitmap) to point to any component you create or onboard.
...
First of all, decide which component you aim to onboard and "liberate" from the project. Know that here, if the component has any dependencies, you will likely to need to onboard them as well, or have them as part of the component. Once you had a target in mind, create a new Bit component:
...
Alternatively, you can use the `bit add` command to onboard components to Bit. Note that with this flow you still have to decouple the component and ensure all dependencies are Bit components as well.
