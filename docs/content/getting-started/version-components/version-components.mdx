...
If you see all component status as `ok`, Bit can version the component.

To tag the component, use `tag` command and specify the component ID:

```bash
bit tag envs/my-react --message "my changelog message"
```
...
:::note
If you changed the default scope in the `workspace.json`, make sure you also modify the imports in the components to correspond with your company and scope name.
:::
...
```shell
bit tag pages/welcome --ver 1.0.0 // Make sure you modify the component before running this command.
```
...
```bash
bit checkout 0.0.1 pages/welcome
```
