...
A component in Bit has its own revision history. This history contains code, dependencies and configuration changes. This reversion history can be shared through a remote Bit server.

Each revision has to be completely isolated from the workspace. To evaluate if a state of a component can be saved, run:
...
During the snap process Bit runs the component's build pipeline to ensure it's fully functional. A snapshot will have it's own unique hash to be used when referring to that state in time.

Read more about [snapshots](/docs/components/snaps) and how to use them.
...
### Comparing with past implementation
...
While the UI lets you view a component version from the past, using the CLI you can checkout a snap or a version of a component to your workspace. To do so, run:
