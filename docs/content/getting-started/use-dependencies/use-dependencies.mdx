...
[`Package name`](/docs/packages/package-name) for a component is by default computed from its [Component ID](/components/component-id). You can customize it to your liking with other conventions.
...
[External dependencies](/docs/dependencies/external-dependencies) are all dependencies listed as part of the workspace [Dependency Policy](/docs/dependencies/dependency-policies) and are installed with the `install` command.
...
Depending on the type of dependency you have, Bit has a different flow to update it.

### Dependencies on local components

When a component depends on another component in the same workspace, Bit resolves the dependency according to the state of your workspace. This means that when you update a component, you essentially update all its dependents.
...
`devDependencies` is contextual. Some components many use a dependency for their testing, and others will need it to operate. Managing the type of dependency can be a pain. Bit solves this by treating `import` and `require` statements from files classified as [Dev Files](/docs/components/dev-files) differently. Any dependency resolved from a dev-file is marked as `devDependencies` for the specific component. `bit show` output lists which are the dev-files of a component.
...
Bit offers an automatic way to manage your dependency graph. However, you can take control to your own hands and override any automated behavior. To control and configure different dependency resolution to some components you need to use the [Workspace Variants Aspect](/docs/workspace/variants).  
