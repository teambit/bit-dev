...
The dependency policy defines [External dependencies](/dependencies/external-dependencies) that are to be installed in that workspace. Note that a workspace policy does not automatically add a dependency to a component.
