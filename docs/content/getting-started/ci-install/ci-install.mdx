...
### Components on [bit.cloud](https://bit.cloud)

All components hosted on Bit.cloud are accessible from the `https://node.bit.cloud` domain with your account as a `registry`. To install components directly from there configure the following in your `.npmrc` file:
...
If you are publishing your components on a remote registry or integrated your registry to cache exported components from [bit.cloud](https://bit.cloud) configure your `.npmrc` with that information.
...
- name: init bit.cloud
  run: |
    ...
    npm config set @<account-name>:registry=https://node.bit.cloud
