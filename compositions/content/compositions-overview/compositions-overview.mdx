...
The Compositions Aspect is set, by default, to detect all `*.composition?(s).*` files from the [component directories](/workspace/component-directory). It uses the preview Aspect to display the content of composition files with the `*.{t,j}s?(x)` extension. Learn more about this process. <!-- TODO -->.
