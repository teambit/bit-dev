...
The name for the [link file](/dev-services/preview/overview#link-files) <!-- TODO --> and corresponding URL parameter (which will be used to fetch this type of previews).
...
A function that gets an [`ExecutionContext`](https://bit.cloud/teambit/envs/envs/~code/context/context.ts) with data regarding the Env's instance, and returns a path to the preview template which would be used to render the files returned by `getModuleMap` (for example, the [docs template](/dev-services/docs/docs-templates)).
...
See ['Register files for bundling'](#register-files-for-bundling), to learn how to register the template to the Preview aspect.
