...
<ComponentCardDisplay
  componentIds={['teambit.preview/preview']}
...
1. Collects all relevant files for a preview and registers them in a [link file](#link-files). The collected files include component files (using a specific glob pattern), templates, setup code, etc. To learn how to extend this step, see ['Register files for bundling'](/dev-services/preview/register-custom-preview#register-files-for-bundling).
...
6. Retrieves and renders the relevant preview in the browser, according to the URL sent by the browser. To learn how to extend this step, see ['Register files for routing and rendering'](/dev-services/preview/register-custom-preview#register-files-for-routing-and-rendering).
