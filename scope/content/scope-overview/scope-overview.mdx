...
Scopes support various methods for consuming components, from adding as buildtime dependencies all the way to runtime integrations. The consumer then decide how whats the best approach for their use case.

<Image width="70%" src={componentTypes} />

## Team collaboration using components

Scopes provides visibility to components built by other teams, this is the first step towards better collaboration. Developers can find, use and depend on components by others. This high level of visibility is a drive for better collaboration, as teams see they don't need to rebuild components and re-invent their wheel. It is similar to how teams build with microservices and build team communication and collaboration by using each-other's APIs.  
With `bit import` consumers can access to any component's source code and contribute change and collaborate on specific components.
...
Hosted scopes form a distributed network, where each scope is connected to the other scopes according to component dependencies. A scope stores its dependencies as well, so in case of a failure in the network, you are always safe.  
