...
Instructions on how to self host your own Scope are [this page](/docs/scope/running-a-scope-server).
...
When working with Bit.cloud there's no need for any additional local configuration to export components. Bit resolves the scopes from Bit.cloud with a predefined [resolve function](TODO). This means you only need to set your scope in `workspace.jsonc`.
