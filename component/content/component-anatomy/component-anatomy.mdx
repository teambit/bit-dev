...
Main file exposes the public API of the component. Read more about it [here](/components/component-main-file).
...
Bit marks which configuration set each of the files as a dev-file. [read more](/components/dev-files) on how to manage component dev-files.

## Dependencies

Component dependencies are managed by the [dependency resolver](https://bit.cloud/docs/dependencies/dependencies-overview). This tool reads through all `import` and `require` statement in the component source code, and maps them to components or installed packages.
...
`dependencies` and `devDependencies` are contextual to the component. While one component may use a dependency for its documentation, another component may use it for its implementation. This is why the dependency resolver is managing dependency type on a per-component basis. More specifically, by understanding in which file type the dependency is used.
...
Different aspects can add data into a component.
