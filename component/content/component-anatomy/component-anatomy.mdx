...
The 'source code' includes the component's [main file](./components/component-main-file), implementation files and [development files](./components/dev-files). A file is considered part of the component if it is under the same [Component Directory](./workspace/component-directory).
...
```bash
bit show teambit.base-react/navigation/link
```
...
A [Component's configuration](./components/component-config) consists of the Aspects it uses and their corresponding configs.
For example:
...
The following output displays the list of aspects applied on this component. The effects of these aspects can be seen in other types of component data. For example, the 'compositions' aspect defines the `.compositions.*` files as dev file.
It's also responsible for generating the artifacts that make these compositions renderable by the browser.
...
Configured aspects can also persist and retieve metadata on components.

## Artifacts

[Artifacts](./dev-services/builder/build-artifacts) are generated during the component [build](./dev-services/builder/build-pipelines). Artifacts are persisted when a Component is snapped or tagged. Each Component version holds the artifacts that were generated as part of its build.
