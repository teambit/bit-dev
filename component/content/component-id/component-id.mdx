...
Bit arranges your workspace directories according to the IDs of components in that workspace. It destructures the Component IDs and creates a directory tree that represents the hierarchy found in these IDs. This makes the workspace easier to navigate as it creates symmetry between the logical structure of components and their physical location.

For example, when creating a component with `my-org.my-scope/forms/add-product` Component ID, Bit generates the following directories:
