...
- **Learn more about adding Bit to your CI, [here](/cicd/cicd-setup-with-bit)**
...
To explore the previous versions of a component in the Workspace/Scope UI, [see here](/components/inspecting-components#workspace-scope-ui).

## Actions executed by the 'tag' command

### 1. Runs the environment's 'build pipeline'

The 'build pipeline' is a series of tasks defined by the environment. In our case, we've set all our components to [use the React environment](/envs/pre-configured-envs/react) which has, as a default, two tasks in its build pipeline:
...
> As with any other service provided by the environment, the '[build pipeline](/builder/build-pipelines)' can too be extended and customized.
...
The above example uses the `--persist` flag to perform a 'hard tag'. In most cases, you would not want to commit changes (and later on, export) components directly from your local environment. It is usually preferable to use 'soft tag' to **propose** a new version and let your [CI](/cicd/cicd-setup-with-bit) set a new version with the committed changes (using 'hard tag')

```shell title="soft-tag"
