...
A Component's configuration determines the properties of a [Component](/components/component-overview), like its [package name](/packages/package-name), [dependencies](/dependencies/configuring-dependencies), [dev files](/components/dev-files), and more.
It is essentially, a list of [Aspects](/extending-bit/aspect-overview) that are applied on a Component and their corresponding configs. One very prominent Aspect is the specific [Env](/envs/envs-overview) a Component uses.
...
Aspects are applied on a Component using its [component.json](/components/component-json) and using [Variants](/workspace/variants). Core Aspects are applied by default and will not be listed in either of the two.
