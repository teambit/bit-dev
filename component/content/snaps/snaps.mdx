...
The snap command runs the Component's Build and Snap pipelines before it creates a new version. The generated version is stored in the Local Scope, where it can be exported to a Remote Scope.
...
Run the following to undo the latest (local) Component snap:
