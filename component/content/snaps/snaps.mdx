...
In Bit you manage changes across connected components, this guide follows how snapping changes work for components that have dependencies on each other.

The guide follows sharing changes for `ui/text` and `ui/heading` in a workspace with 4 connected components.

<Image src={snapOverview} width="50%" />
...
During the `snap` process, Bit runs the full component [build pipeline](/dev-services-overview/builder/build-pipelines), as defined in its environment.  
...
author: my-user <itay@mydomain.com>
...
Read more [here](/components/exporting-components).
