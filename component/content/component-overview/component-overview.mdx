...
Each Component has a [Component ID](/components/component-id) and contains [Source Code](/components/source-code), [Artifacts](/components/artifact), [Configuration](/components/component-config) and [Metadata](/components/metadata). 
Components are composed to each other with [Dependencies](/dependencies/overview).
