...
The _main file_ exposes its Component's API. The main file makes the Component's code more maintainable and comprehensible by providing a single entry point.
...
1. File with the same name as the component's directory name.

Bit registers the main file in the `.bitmap` file. For example:
