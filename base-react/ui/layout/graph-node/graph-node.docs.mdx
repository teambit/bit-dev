  ...
  return (
    <div>
      {Object.values(nodeLayout).map((i) => {
          const highlightedDefaultClass = i && i.includes('graph-node_colSpan--4-3') ? 'bold' : 'normal'
...
Passing `undefined` or `null` in both `row` and `col` will create a class as can be seen in the example. You can use this behavior to find and target this node.
